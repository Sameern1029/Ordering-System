...
def def_full_file_reader():                                                                        
    file_foods = open('config'+navigator_symbol+'list_foods.cfg', 'r') 
    for i in file_foods: 
        list_foods.append(str(i.strip())) 
    file_foods.close()

    file_drinks = open('config'+navigator_symbol+'list_drinks.cfg', 'r') 
    for i in file_drinks:
        list_drinks.append(str(i.strip()))
    file_drinks.close()

    file_services = open('config'+navigator_symbol+'list_services.cfg', 'r') 
...
def def_report():
    while True:
        print("*" * 33 + "REPORT" + "*" * 33 + "\n")
        file_report = open('config'+navigator_symbol+'report.cfg', 'r').read() 
        print(file_report)
        print("\n(M) MAIN MENU          (E) EXIT\n" + "_" * 72)
        input_1 = str(input("Please Select Your Operation: ")).upper()
        if (input_1 == 'M'):
...
def def_payment():
    while True:
        ...
        if (input_1 == 'P'):
            ...
            file_report = open('config'+navigator_symbol+'report.cfg', 'a') 
